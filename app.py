...
try:
    ...
    # 1. Obtenemos el valor máximo (si existe)
    val_max = data_mapa_raw['dist_media'].dropna().max()

    # 2. Si val_max es NaN, le ponemos un 100 por defecto para que el slider no rompa
    max_dist_data = float(val_max) if pd.notna(val_max) else 100.0

    # 3. Ahora el slider siempre es seguro
